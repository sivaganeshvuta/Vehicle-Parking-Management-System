...
class DBOperation():
    ...
    def __init__(self):
        ...
        file.close()
        self.mydb=mysql.connector.connect(host="localhost",user="root",passwd="",database="vpms_py")
    ...
    def exitVehicle(self,id):
        cursor=self.mydb.cursor()
        currentdata = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("UPDATE slots set is_empty='1',vehicle_id=NULL where vehicle_id='"+id+"'")
        self.mydb.commit()
